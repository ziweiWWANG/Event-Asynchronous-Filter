function output_img(log_intensity_now_,img_idx_now,use_median_filter,safety_offset,post_process,folder,P,log_intensity_state_,output_gt_HDR)
    ...
    if output_gt_HDR
        ...
        % tonemapping for display only
        save(sprintf([folder '/raw_' num2str(img_idx_now) '.mat']), 'td_img');
        [td_img] = tonemap_hdr(td_img);
    elseif post_process == 1
        tmp = sort(td_img(:)); 
